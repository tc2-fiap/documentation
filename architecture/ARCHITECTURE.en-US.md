...
### 5.4 Cross-cutting: JWT revocation on logout (a third event, outside both flows)

`POST /api/users/logout` (`users-api`) publishes `TokenRevokedEvent { Jti, ExpiresAtUtc }` off the caller's own token claims. Unlike the two flows above, every one of the six services consumes it — including `catalog-api` and `platform-api`, which otherwise sit outside the broker entirely (§1, §6). JWTs here are stateless, validated independently by each service off a shared signing secret (`notes.md` 19), so without this a revoked token would keep authenticating until its natural expiry (`Jwt:ExpiryMinutes`, 60 by default) regardless of logout. Each service holds its own `ITokenRevocationStore`/`InMemoryTokenRevocationStore` — a per-pod `ConcurrentDictionary<Jti, ExpiresAtUtc>`, swept opportunistically on read rather than by a background timer — checked in a new `OnTokenValidated` hook added to the already-duplicated `AuthenticationExtensions.AddJwtAuthentication`. Being in-memory and per-pod, a restart forgets active revocations, but the exposure window is bounded to whatever's left of the affected token's natural lifetime, never indefinite (`notes.md` 84).

Each of the six services declares its own explicit `cfg.ReceiveEndpoint("<service>-token-revoked", ...)`, the same convention `orders-api`'s/`payments-api`'s existing consumers already use, and for a reason discovered live during this feature's own rollout: MassTransit's default endpoint-naming convention derives a queue name from the consumer/message type alone, so six identically-named `TokenRevokedConsumer` classes without an explicit name would land on one shared queue and compete for messages round-robin instead of each service getting its own copy (`notes.md` 84).
...
**Account safety.** Self-registration always creates a `Player` (`notes.md` 26) — the only route to an `Admin` is an existing one promoting someone else — so `PUT /api/users/{id}/role` and `DELETE /api/users/{id}` both reject `id == callerId` outright, closing off the one way this system could otherwise strand itself with zero admins. Role changes now also publish `RoleChangedEvent { UserId, OldRole, NewRole, ChangedByUserId }`, persisted into the same `UserEvent` audit log `UserCreatedEvent` already uses, so it surfaces in `/admin/events` (§ above) with no new admin-facing code. Separately, `POST /api/users/login` locks an account for 15 minutes after 5 consecutive failed attempts (tracked as `FailedLoginAttempts`/`LockedUntilUtc` on `User` itself, reset on a successful login) — per-account, not per-IP. Registration's `Password` rule requires 12+ characters across all four character classes (upper/lower/digit/special) plus rejection of a small common-weak-password denylist, up from an earlier 8-characters-only rule. See `notes.md` 83, 84.
...
- **Secrets audit**: `helm template | grep -iE "password\|secret\|apikey"` surfaces literal values only inside `Secret` resources themselves — every `Deployment` env var referencing one uses `secretKeyRef`, confirmed across all seven charts. Those `Secret` resources are still populated from plaintext values committed in `orchestration/values.yaml` (the JWT signing key, every Postgres/RabbitMQ password, the seeded Admin/Player passwords) — reviewed directly and kept as a deliberate tradeoff for this college project, not an oversight: a working, zero-config `helm install` with known, documented credentials outweighs secret rotation for a non-production, localhost-only cluster nobody else can reach (`notes.md` 93).
- **Network segmentation**: `kind`'s default CNI (`kindnet`) doesn't enforce `NetworkPolicy` at all — confirmed via research before relying on it — so the cluster now provisions with it disabled and Calico installed instead (`orchestration/kind/cluster-config.yaml`'s `networking.disableDefaultCNI`, `GETTING_STARTED.md` step 2). Four `NetworkPolicy` resources (`orchestration/templates/network-policy.yaml`) enforce a default-deny-ingress baseline for the namespace, then explicit allows matching exactly the [service topology diagram](../diagrams/service-topology.md)'s own arrows: ingress-nginx → the 7 app pods, `orders-api` → `catalog-api` (its one direct pod-to-pod HTTP call), the 5 DB-backed services → Postgres, the 6 event publishers/consumers → RabbitMQ.
- **Resource limits**: every one of the 9 workloads (7 app pods, Postgres, RabbitMQ) now declares CPU/memory `requests`/`limits` — modest values sized for a local `kind` node, not load-tested.
- **Capability dropping**: all 7 application containers, and their `wait-for-postgres` init containers, run with every Linux capability dropped (`capabilities.drop: ["ALL"]`) alongside the existing `allowPrivilegeEscalation: false`. `postgres`/`rabbitmq` deliberately don't get this — both official images' own entrypoints need root-level setup (`chown`, `su-exec`) before de-escalating to an unprivileged user internally, confirmed live by the startup failure that resulted from stripping it (`repos/documentation/discovers/DISCOVERIES.en-US.md` 17).
- **Frontend security headers**: `frontend/nginx.conf` serves a real `Content-Security-Policy` — scoped to exactly what the SPA loads (Google Sign-In's script/iframe from `accounts.google.com`, the System Health page's `api.github.com` compare call, external catalog cover art, the PIX QR code's `data:` image) — plus `X-Content-Type-Options`, `X-Frame-Options: DENY`, and `Referrer-Policy`.
