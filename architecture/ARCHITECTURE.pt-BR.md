...
### 5.4 Transversal: revogação de JWT no logout (um terceiro evento, fora dos dois fluxos)

`POST /api/users/logout` (`users-api`) publica `TokenRevokedEvent { Jti, ExpiresAtUtc }` a partir das próprias claims do token de quem chama. Diferente dos dois fluxos acima, todos os seis serviços consomem esse evento — incluindo `catalog-api` e `platform-api`, que fora isso ficam inteiramente fora do broker (§1, §6). Os JWTs aqui são stateless, validados de forma independente por cada serviço a partir de um segredo de assinatura compartilhado (`notes.md` 19), então sem isso um token revogado continuaria autenticando até sua expiração natural (`Jwt:ExpiryMinutes`, 60 por padrão), não importa o logout. Cada serviço mantém seu próprio `ITokenRevocationStore`/`InMemoryTokenRevocationStore` — um `ConcurrentDictionary<Jti, ExpiresAtUtc>` em memória, por pod, limpo de forma oportunista na leitura em vez de por um timer em segundo plano — checado num novo hook `OnTokenValidated` adicionado ao já duplicado `AuthenticationExtensions.AddJwtAuthentication`. Por ser em memória e por pod, um reinício esquece as revogações ativas, mas a janela de exposição fica limitada ao que resta da vida útil natural do token afetado, nunca indefinida (`notes.md` 84).

Cada um dos seis serviços declara seu próprio `cfg.ReceiveEndpoint("<service>-token-revoked", ...)` explícito, a mesma convenção que os consumidores já existentes do `orders-api`/`payments-api` usam, por um motivo descoberto ao vivo durante a própria implantação dessa funcionalidade: a convenção padrão de nomeação de endpoint do MassTransit deriva o nome da fila só do tipo de consumidor/mensagem, então seis classes `TokenRevokedConsumer` com o mesmo nome, sem um nome explícito, cairiam numa única fila compartilhada e competiriam pelas mensagens em round-robin em vez de cada serviço receber sua própria cópia (`notes.md` 84).
...
**Segurança da conta.** O autocadastro sempre cria um `Player` (`notes.md` 26) — o único caminho até um `Admin` é um já existente promovendo outra pessoa — então `PUT /api/users/{id}/role` e `DELETE /api/users/{id}` recusam `id == callerId` de cara, fechando a única forma de o sistema ficar sem nenhum admin. Mudanças de role agora também publicam `RoleChangedEvent { UserId, OldRole, NewRole, ChangedByUserId }`, persistido no mesmo log de auditoria `UserEvent` que o `UserCreatedEvent` já usa, então aparece em `/admin/events` (acima) sem nenhum código novo no admin. Separadamente, `POST /api/users/login` bloqueia uma conta por 15 minutos depois de 5 tentativas de login seguidas com falha (rastreadas como `FailedLoginAttempts`/`LockedUntilUtc` no próprio `User`, zeradas num login bem-sucedido) — por conta, não por IP. A regra de `Password` do cadastro agora exige 12+ caracteres cobrindo as quatro classes de caractere (maiúscula/minúscula/dígito/especial) mais a rejeição de uma pequena lista de senhas fracas comuns, substituindo uma regra anterior de só 8 caracteres. Veja `notes.md` 83, 84.
...
Só o `users-api` precisou de uma tabela nova (`UserEvent`) — era o único serviço sem nenhum registro de que o `UserCreatedEvent` havia sido publicado. `orders-api`, `payments-api` e `notifications-api` já tinham uma tabela com tudo o que era necessário (`OrderEvent`, `Payment` e `Notification`, respectivamente); cada um só ganhou uma nova consulta paginada e sem escopo por pedido sobre dados que já persistia. O `catalog-api` está ausente dos dois diagramas — ele não publica nem consome nenhum evento do qual esses diagramas tratem (ele consome o `TokenRevokedEvent`, transversal e sem relação com isso — §5.4).
