...
## 94. `DEPLOY_VERIFICATION.md` gained a section on adding a new direct service-to-service call under the new `NetworkPolicy`

**Context.** Follow-up questions after entry 93 shipped (does `orders-api` reach `payments-api`? can the whole mesh be opened back up?) surfaced a real documentation gap: `ARCHITECTURE.md` states which pairs are allowed *today*, and entry 93 itself records the historical bug (`orders-api → catalog-api` silently hanging until a rule was added), but nothing forward-looking told a future reader what happens if *they* add a new direct call, or how to check/fix it — and `DEPLOY_VERIFICATION.md`, the doc whose whole job is troubleshooting the deployed system, had zero mentions of `NetworkPolicy` at all.

**Decision.** New section, both languages, placed after the existing `/health`/`/version`/port-forward section (same "how traffic actually reaches things in this cluster" territory): explains that a blocked call doesn't error clearly, it just times out exactly like a slow or dead service would, and to check `kubectl get networkpolicy -n fiap-games` before assuming anything else; gives the exact YAML shape to copy from `allow-orders-to-catalog` for a new pair; and gives the "loosen it for every service at once" alternative discussed with the user directly, with the tradeoff stated plainly (a compromised pod regains the ability to reach everything, same as before entry 93).

**Revisit if:** a real feature ever needs the all-to-all option for real — right now it's documented as an available choice, not something anything in this system actually uses.

## 95. `DEPLOY_VERIFICATION.md` gained a section on rebuilding every service locally from the published GHCR images — and caught a real `-n fiap-games`-vs-`-n default` bug in every existing `helm upgrade` example while writing it

**Context.** Requested directly: a section covering the opposite of "test my own local code" — pull the images CI already published, load them locally, and pin the cluster to that exact snapshot instead of trusting a live `Always` re-pull. Useful for GHCR flakiness, for guaranteeing you're looking at the exact bits you just inspected, or as a faster bulk refresh than 7 separate live pulls.

**A real, pre-existing bug surfaced while verifying the new section live, not invented by it.** Every `helm upgrade` example already in this file (both "Testing a local, uncommitted change" and "Testing every service's local changes at once", entry 90) used `-n fiap-games` — which fails outright: `Error: UPGRADE FAILED: "fiap-games" has no deployed releases`. The release itself lives in the `default` namespace (`helm list -A` confirms it — `GETTING_STARTED.md`'s own `helm install fiap-games .` never passes `-n` at all, so the release metadata lands wherever the current context's default namespace is), even though every *resource* the chart renders lands in the `fiap-games` namespace via `.Values.global.namespace`. The two are independent: `-n` on a `helm upgrade`/`install` command picks the namespace Helm stores the *release* in, not the namespace its templates target. This had apparently never actually been re-run against the doc's own literal text since it was written — every previous verification pass ran the equivalent command with `-n default` in the terminal, and the doc itself just never got cross-checked against that until this pass tried to follow its own new section literally.

**Fixed**: all three existing `-n fiap-games` occurrences (two `helm upgrade` code blocks, one `--reset-values` mention) in both `DEPLOY_VERIFICATION.en-US.md` and `.pt-BR.md`, corrected to `-n default` — `kubectl` commands elsewhere in the same file correctly keep `-n fiap-games`, since those really do target the resource namespace, not the release namespace.

**Verified live, full round-trip, with the corrected flag**: pulled and `kind load`ed `platform-api`'s published GHCR image fresh (removed the local copy first to force a real pull), flipped only its `pullPolicy` to `IfNotPresent` via `helm upgrade fiap-games . -n default --set platform-api.image.pullPolicy=IfNotPresent` — pod event showed `Container image "ghcr.io/tc2-fiap/platform-api:latest" already present on machine`, no `Pulling image` at all. Reverted with `--reset-values` — the very next pod showed a real `Pulling image`/`Successfully pulled` pair again, confirming the round trip. All 9 pods stayed `Running`/healthy throughout.

**Revisit if:** never, structurally — this is the same class of "a doc example was never actually re-run" drift entry 92 already caught once; the fix is complete as written.

## 96. System Health's Restart button gave zero feedback after a successful restart — looked broken, wasn't

**Context.** Reported directly: the Restart button "isn't working, seems like taking a lot of time." Investigated by checking `platform-api`'s own request logs rather than guessing — the actual request the user made (`POST /api/platform/admin/services/users-api/restart`) completed server-side in **131ms**, returned `204`, and the pod genuinely restarted (confirmed via `kubectl get pods`, a new `users-api` pod a few minutes old). The backend was never slow; a second live test (`catalog-api`) came back in 10.8ms. So "taking a lot of time" wasn't a real backend delay — it was the *absence of any visible change* being mistaken for one.

**Root cause, found by reading the code, not assumed.** `handleConfirmRestart` in `AdminSystemHealthPage.tsx` closed the confirmation modal on success but never refetched anything — the Services and Pods tables kept showing whatever data was already on screen before the click. A rolling restart takes real time to actually replace a pod; with no refresh, the page gives the user nothing to look at that would prove the action did anything, indistinguishable from the request having silently failed. Separately, `ConfirmModal.tsx` (shared by this page, `AdminGamesPage`, and `LibraryPage`) already had a `busy` prop disabling its buttons mid-request, but rendered no visual indicator at all — just two static, disabled buttons with unchanged labels.

**Fixed, two independent things:**
- `handleConfirmRestart` now calls `fetchAll()` again after a successful restart, the same refresh a manual "Refresh" click already does — the Pods table updates in place (no loading skeleton, matching how "Refresh" already behaves, since `loading` is never reset to `true` for either path).
- `ConfirmModal.tsx` gained a small CSS spinner (`.btn-spinner`, `currentColor`-based so it matches each button variant) rendered on the confirm button whenever `busy` is true, plus `aria-busy`. Fixed at the shared-component level rather than one-off in `AdminSystemHealthPage` — `AdminGamesPage`'s delete and `LibraryPage`'s remove both already pass `busy` too and get the same improvement for free, no extra work needed in either.

**Verified live in a real browser** (Claude in Chrome, admin session): clicked Restart on `users-api`, confirmed, watched the Pods table actually update to show a brand-new pod (`users-api-6bd4f866bf-jq27t`, age `0m`, `Running`, `1/1`) with no manual refresh needed — the exact feedback that was missing before. `npm run build`/`npm run lint` clean; verified against a locally-built image via the documented override pattern (entry 95), then reverted.

**Revisit if:** a rolling restart ever needs to be watched to actual completion (not just "triggered") — right now `fetchAll()` fires once, immediately after the `204`, which is enough to prove the action registered but may still show the old pod mid-transition; a slow-motion demo could still misread that as nothing having happened. Not built now since a single refresh already resolves the reported symptom.

## 97. The restart endpoint returns a real body instead of a bare `204`

**Context.** Asked directly, after confirming (via Chrome DevTools) that the restart request's Response tab said "Failed to load response data — No data found for resource with given identifier." That's Chrome's confusing way of saying there was never anything to show: `Results.NoContent()` is a genuine, spec-correct `204` — zero bytes, by design, not a bug. Confirmed live with a raw `curl -i`: `HTTP/1.1 204 No Content`, no body, and cross-checked the exact same thing server-side in `platform-api`'s own request log and `ingress-nginx`'s access log, both agreeing. The user asked to add a body anyway, since "confirm it worked, with something to look at" is a reasonable want even when `204` was never actually broken.

**Decision.** `IDeploymentService.RestartAsync` now returns `Task<Result<RestartResponse>>` instead of `Task<Result>` — a new `Application/Dtos/RestartResponse.cs` (`Service`, `RestartedAtUtc`), following the same `sealed record` convention as `PodResponse`. `ResultExtensions.ToHttpResult<T>` already existed and already mapped a successful `Result<T>` to `Results.Ok(result.Value)` — no new extension code needed, the generic overload just started being used. Success is now `200 OK` with `{"service":"...","restartedAtUtc":"..."}`; the failure path (`400` with a `ProblemDetails` body for an unknown name) is unchanged.

**Verified live**: `dotnet test` 13/13 (via the SDK container, no local `dotnet` on `PATH` in this environment), `docker build` clean, and a fresh `curl -i` against the running cluster showing the real `200` + JSON body, plus the unknown-name path still correctly `400`.

**Revisit if:** never — this was a deliberate API-shape choice, not a bug fix; the old `204` behavior wasn't wrong, just less informative than asked for.

## 98. Stop/Start a service from System Health — scaling to 0/1 replicas, not a rolling restart

**Context.** Requested directly: "add stop and start pod from system-health." A rolling restart (already built) never drops replica count below 1 — this needed to be a genuinely different action, an actual scale-to-0/scale-to-1, done via the same `PatchNamespacedDeploymentAsync` call the restart code already used, just patching `spec.replicas` instead of the pod-template annotation. `kubectl delete pod` was never on the table — the Deployment controller would just recreate a deleted pod instantly, so "stop" only means anything at the Deployment level.

**RBAC needed zero changes — checked directly against `platform-api/k8s/templates/rbac.yaml` before writing any code, not assumed.** The existing `platform-api-deployment-restarter` Role already grants `patch` on `deployments`, `resourceNames`-scoped to exactly the 7 legitimate Deployments — the same verb, same resource, same rule covers a `spec.replicas` patch as much as a pod-template-annotation one. The whole feature shipped with no infrastructure changes at all.

**Two services got special handling, decided directly with the user before writing code:**
- **`platform-api` is hard-blocked from Stop, in `DeploymentService` itself** — not just the UI. It's what serves this very endpoint; stopping it would brick the admin API with no recovery path but `kubectl`. `StopAsync` checks for it explicitly before ever calling the restarter, on top of the shared allowlist check; `StartAsync`/`RestartAsync` don't block it, since neither can ever put it at 0 replicas in the first place. The frontend's `platform-api` row never renders a Stop button either, matching the block on both sides rather than trusting one.
- **`frontend` is allowed, with a warning in the confirm dialog** — and the reasoning was verified live, not just asserted: stopped `frontend` (`kubectl get pods` confirmed zero), then confirmed `GET /` genuinely returns `503` (a fresh page load really does need the pod) while `POST /api/platform/admin/services/frontend/start` still succeeded and brought the pod back — because that call goes straight to `platform-api` through the Ingress's `/api/*` routing, never touching `frontend`'s own pod. An already-open tab can recover itself; a reload or a closed tab while stopped cannot, which is exactly what the dialog's warning says.

**Backend**: `IDeploymentRestarter`/`KubernetesDeploymentRestarter` gained `ScaleAsync(name, replicas)` (same class as `RestartAsync`, not a new interface — same "mutate this Deployment via patch" responsibility). New `Application/Dtos/ScaleResponse.cs` (`Service`, `Replicas`, `UpdatedAtUtc`), shared by both directions. `DeploymentService` gained `StopAsync`/`StartAsync`, both funneling through a private `ScaleAsync` that does the shared allowlist check. Two new routes, `POST /api/platform/admin/services/{name}/stop` and `.../start`, same `RequireAuthorization(Admin)` shape as restart. 14 new tests in `DeploymentServiceTests.cs` (allowlisted stop/start per service, the `platform-api`-stop rejection specifically, unknown-name rejection for both) — 27/27 passing.

**Frontend**: the three near-identical `restartTarget`/`restarting`/`handleConfirmRestart` pieces of state became one `pendingAction: {action: 'restart'|'stop'|'start', service} | null` + a single dispatching handler, still driving the one shared `ConfirmModal` — avoided tripling the modal-open/busy/error/refetch plumbing for what's the same shape of action three times over. Running/stopped state is derived from data already being fetched (`pods.some(p => p.application === name)`) — no new endpoint, no new poll; a scale-to-0 genuinely leaves zero pods for that app, and a restart's rolling update (new pod up before the old one terminates, for a single replica) never transiently shows zero, so the two actions can't be confused. Status column gained a third state, "Stopped" (amber `pending` styling, distinct from the red `unreachable` used for a genuine fault), for exactly this case — technically `/version` failing either way, but a deliberate Stop reads better as "Stopped" than "Unreachable."

**Verified live**: `dotnet test` 27/27, `docker build` clean for both `platform-api` and `frontend`, `npm run build`/`npm run lint` clean. Against the running cluster: `platform-api` stop correctly `400`s ("platform-api cannot be stopped from this page"); `notifications-api` stop/start round-tripped for real (`kubectl get pods` showed zero, then one, matching each call); the `frontend` self-recovery mechanism confirmed exactly as reasoned above. The browser extension wasn't connected during this session, so the actual button/modal UI wasn't clicked through live — every behavior it depends on (the API contract, the running-state derivation logic, the self-recovery path) was verified directly instead.

**Revisit if:** the browser extension is available in a later session — worth a real click-through to catch anything the API-level verification couldn't (e.g. the actions-cell layout with 1 vs 2 buttons, the frontend warning text rendering correctly inside the modal).
