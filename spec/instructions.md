...
- **The only service with Kubernetes RBAC.** A dedicated `ServiceAccount` bound to two namespaced `Role`s — never a `ClusterRole` — is the sole cluster-API credential anywhere in the system: `get`/`list`/`watch` on `pods`, plus a narrowly `resourceNames`-scoped `patch` on exactly the seven service `Deployment`s (added for the admin System Health page's Restart button, `notes.md` 88) — the RBAC layer itself blocks it from touching anything else, e.g. `postgres`/`rabbitmq`, not just application code. Every other service's Kubernetes footprint is "a Deployment that runs a container"; this is the one place a service is *aware* it runs on Kubernetes at all.

Every service, including PlatformAPI itself, also exposes both a bare `GET /version` (unauthenticated, reachable only via `kubectl port-forward`, not under `/api/*`) and an Admin-gated `GET /api/<prefix>/version` (reached through the Ingress like any other route) returning `{ sha, buildTime }` — the same handler registered at both paths. Each Dockerfile computes this itself (`git rev-parse HEAD`, baked into a `build-info.json` file at image build time) rather than requiring a `--build-arg` to be passed in — see `DEPLOY_VERIFICATION.md`. The dashboard's Services table calls each service's admin-gated version endpoint (`Promise.allSettled`, so one unreachable service doesn't blank the page), plus GitHub's compare API directly from the browser to flag commits not yet built (`notes.md` 88), alongside PlatformAPI's Pods table — independent data sources on the same page, not one endpoint doing everything.
