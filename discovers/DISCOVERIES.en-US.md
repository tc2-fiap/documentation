...
## 14. Kubernetes' liveness/readiness split is two much older ideas glued together, not a Kubernetes invention

Every service's Deployment (e.g. `orders-api/k8s/templates/deployment.yaml`) declares a `readinessProbe` and a `livenessProbe` against the same `/health` endpoint, polled on independent schedules (`periodSeconds: 5` and `10`). The two questions they answer are actually two separate, decades-older ideas Kubernetes just gave a shared HTTP contract to. **Readiness** — should this instance receive traffic right now — is the same question a load balancer's health check has asked since at least the 1990s: an L4/L7 balancer pulling a backend out of rotation the moment it stops answering, and putting it back the moment it does. **Liveness** — should this instance be killed and replaced — is the "let it crash and get restarted by a supervisor" idea from Erlang/OTP's **supervision trees**, described by Joe Armstrong in his 1996/2003 PhD work at Ericsson as the core reliability mechanism behind OTP's fault tolerance, itself building on internal Ericsson telecom-switch practice from the mid-1980s. Kubernetes didn't invent either question — it just standardized both as an HTTP polling contract any container can implement.

## 15. The `@tr`/`@sp` fields on every log line are Dapper's decade-old idea, not a Serilog feature

Every request log line in this system carries a `@tr` (trace ID) and `@sp` (span ID) — the same `@tr` value threads through a catalog price lookup, the order-creation log, and the request-completion log as one causally-linked chain for a single `POST /api/orders` call. No service's `Program.cs` sets these explicitly (`orders-api/src/FiapGames.Orders.Api/Program.cs:16-26` just wires `Serilog.Formatting.Compact.CompactJsonFormatter` to the console) — the formatter surfaces whatever `Activity.Current` already holds, and ASP.NET Core has created one `Activity` per inbound request automatically since .NET Core 3.0, following the **W3C Trace Context** standard (a W3C Recommendation since 2020). The underlying idea — tag a unit of work with a trace ID shared across every service it touches, plus a span ID scoped to just that hop — is Google's **Dapper**, described in a 2010 paper as the tracing infrastructure behind Google's own production services; the same lineage produced Twitter's Zipkin (2012) and, eventually, the OpenTelemetry project that merged OpenTracing and OpenCensus in 2019. Nothing here calls itself "OpenTelemetry," but the trace/span pair on every log line is the same mechanism — just consumed straight from the logs instead of a dedicated tracing backend.
...
- **Dapper** — Google's 2010 distributed-tracing paper: tag a unit of work with a trace ID shared across every service it touches, plus a per-hop span ID, so a request can be followed across service boundaries; ancestor of Zipkin, Jaeger, and OpenTelemetry.
...
- **Supervision tree** — Erlang/OTP's fault-tolerance mechanism: a supervisor process restarts a crashed child rather than trying to prevent every possible crash; the model behind Kubernetes' liveness probe.
...
- **W3C Trace Context** — the W3C standard (a Recommendation since 2020) for propagating a trace ID and span ID across service calls; ASP.NET Core implements it automatically via `Activity`, with no app code required.
